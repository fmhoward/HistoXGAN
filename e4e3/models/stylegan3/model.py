...
from e4e3.torch_utils.ops import conv2d_gradfix, filtered_lrelu, bias_act
...
class Generator(nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        # self.log_size = int(np.log2(self.img_resolution))
        self.n_latent = self.num_ws
    ...
    def forward(self, 
        ...
        if not input_is_latent:
            styles = [self.mapping(s, truncation_psi=truncation, truncation_cutoff=None) for s in styles]

        if len(styles) < 2:
            inject_index = self.num_ws
        ...
        # ws = self.mapping(z)
        image = self.synthesis(latent, **synthesis_kwargs)
