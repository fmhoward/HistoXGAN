...
from e4e3.criteria import id_loss, moco_loss
from e4e3.configs import data_configs
from e4e3.datasets.images_dataset import ImagesDataset
from e4e3.criteria.lpips.lpips import LPIPS
from e4e3.models.psp import pSp
from e4e3.models.latent_codes_pool import LatentCodesPool
from e4e3.models.discriminator import LatentCodesDiscriminator
from e4e3.models.encoders.psp_encoders import ProgressiveStage
from e4e3.training.ranger import Ranger
...
class Coach_Slideflow:
    def __init__(self, opts, prev_train_checkpoint=None, ds_train= None, ds_test = None):
        self.opts = opts
        self.global_step = 0
        ...
        if self.opts.lambda_ctp > 0:
            self.ctranspath = build_feature_extractor('ctranspath', tile_px=512, device = self.device)
        if self.opts.lambda_retccl > 0:
            self.retccl = build_feature_extractor('retccl', tile_px=512, device = self.device) 
        self.l1 = nn.L1Loss().to(self.device).eval()
        ...
        # Initialize dataset
        self.train_dataloader = ds_train
        self.test_dataloader = ds_test
        if self.opts.d_lambda > 0:
            self.d_loss = DISTS().to(self.device).eval()
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:        
                    self.checkpoint_me(None, is_best=False)
                #if self.global_step % self.opts.val_interval == 0 or self.global_step == self.opts.max_steps:
    ...
    def calc_loss(self, x, y, y_hat, latent):
        ...
        if self.opts.d_lambda > 0:
            loss_d = self.d_loss(y_hat, y, require_grad=True, batch_average=True)
            loss_dict['loss_d'] = float(loss_d)
            loss += loss_d * self.opts.d_lambda
        if self.opts.lambda_ctp > 0:
            loss_ctp = self.l1(self.ctranspath(127.5*(y_hat.requires_grad_()+1)), self.ctranspath(127.5*(y.requires_grad_()+1)))
            loss_dict['loss_ctp'] = float(loss_ctp)
            loss += self.opts.lambda_ctp*loss_ctp
        if self.opts.lambda_retccl > 0:
            loss_retccl = self.l1(self.retccl(127.5*(y_hat.requires_grad_()+1)), self.retccl(127.5*(y.requires_grad_()+1)))
            loss_dict['loss_retccl'] = float(loss_retccl)
            loss += self.opts.lambda_retccl*loss_retccl
